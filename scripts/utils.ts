...
export function commonDescriptions() {
  ...
  return [
    web3.utils.fromAscii("Loving"),
    web3.utils.fromAscii("Beloved"),
    web3.utils.fromAscii("Beautiful"),
    web3.utils.fromAscii("Good"),
    web3.utils.fromAscii("Graceful"),
  ]
}

//Portrait Description Rare
export function rareDescriptions() {
  const { web3 } = require("hardhat")
  return [
    web3.utils.fromAscii("Glorious"),
    web3.utils.fromAscii("Immortal"),
    web3.utils.fromAscii("Mighty"),
    web3.utils.fromAscii("Humble"),
    web3.utils.fromAscii("Godess"),
    web3.utils.fromAscii("Invincible"),
    web3.utils.fromAscii("Beauteous"),
    web3.utils.fromAscii("Worshipful"),
    web3.utils.fromAscii("August"),
    web3.utils.fromAscii("Eminent"),
    web3.utils.fromAscii("Heroic"),
    web3.utils.fromAscii("High-Spirited"),
    web3.utils.fromAscii("Fascinating"),
    web3.utils.fromAscii("Illustrious"),
    web3.utils.fromAscii("Twice-Royal"),
  ]
}

//Portrait Description Super-Rare and Legendary
export function superRareDescriptions() {
  const { web3 } = require("hardhat")
  return [
    web3.utils.fromAscii("Glorious and Invincible"),
    web3.utils.fromAscii("Immortal and Godess"),
    web3.utils.fromAscii("Mighty and Beauteous"),
    web3.utils.fromAscii("Mighty and All-Powerful"),
    web3.utils.fromAscii("Dear and Absolutely Beautiful"),
    web3.utils.fromAscii("Sweet and Sovereign"),
    web3.utils.fromAscii("Beautiful and Intrepid"),
    web3.utils.fromAscii("Gracious and Benevolent"),
    web3.utils.fromAscii("Grateful and Proud"),
    web3.utils.fromAscii("Noble and Perfect"),
    web3.utils.fromAscii("Wise and Fair"),
    web3.utils.fromAscii("Fair and Benevolent"),
    web3.utils.fromAscii("All-Powerful and Absolutely Beautiful"),
    web3.utils.fromAscii("Mighty and Sovereign"),
  ]
}
...
export async function getAccount(accountType: string, qtty: number = 0) {
  ...
  if (
  ...
  else if (accountType.toLowerCase() == "developer") return accounts[3]
  else if (accountType.toLowerCase() == "founders") {
    let founders: any[] = []
    if (network.config.nome?.toLowerCase() != "development") {
      console.log("No founders key on live network")
      return founders
    }

    for (let idx = 0; idx < 4; idx++) {
      founders.push(accounts[idx + 4])
    }
    return founders
    ...
    if (qtty >= 1) {
      for (let idx = 0; idx < qtty; idx++) {
        minters.push(accounts[idx + 8])
      }
    } else minters.push(accounts[8])
    return minters
  }
}
